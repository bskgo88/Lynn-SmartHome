...
class DeviceControlCommonWidget extends StatefulWidget {
  final LocationModel location;
  DeviceControlCommonWidget({Key key, this.location}) : super(key: key);

  @override
  _DeviceControlCommonWidgetState createState() => _DeviceControlCommonWidgetState(location); //commonState;
}

class _DeviceControlCommonWidgetState extends State<DeviceControlCommonWidget> {
  String _deviceId = "";
  LocationModel _location;

  _DeviceControlCommonWidgetState(this._location) {
    //print("_DeviceControlCommonWidgetState _location.devices.length : ${_location.devices.length}");
    if (_location.devices.length > 0) {
      _deviceId = _location.devices[0].deviceId;
    }
  }

  /// 하위 기기 목록 위젯에서 호츌 할 수 있도록  Function 자체를 DeviceControlWidget 에 파라미터로 넘겨 준다.
  _changeDevice(String deviceId) {
    //print("_DeviceControlCommonWidgetState changeDevice called!! deviceId : ${_deviceId}");
    int sIdx = widget.location.devices.indexWhere((device) => device.deviceId == deviceId);
    if (sIdx == -1) {
      sIdx = 0;
    }
    print("_changeDevice _deviceId : ${widget.location.devices[sIdx].deviceId}  sIdx : $sIdx");
    String deviceTypeCode = widget.location.devices[sIdx].devicemodel.modeltype.code;
    setState(() {
      _deviceId = deviceId;
  ...
  List<Widget> _buildDeviceControls(BuildContext context) {
    return widget.location.devices.map((device) {
      ...
      return DeviceControlWidget(device: device, isSelected: isSelected, changeDevice: _changeDevice); // _changeDevice Function 을 파라미터로 넘김

      // return BlocBuilder < UserHomeBloc, UserHomeState > (
      // builder: (context, UserHomeState state) {
      // 	return DeviceControlWidget(device: device, isSelected: isSelected, changeDevice: _changeDevice);
      // });

      // return BlocConsumer < UserHomeBloc, UserHomeState > ( ////BlocConsumer 사용 - listener 사용하기 위하여
      // 	listener: (context, state) {
      // 		print("_buildDeviceControls BlocConsumer listener!!");
      // 		setState(() {});
      // 	},
      // 	builder: (context, UserHomeState state) {
      // 		return DeviceControlWidget(device: device, isSelected: isSelected, changeDevice: _changeDevice);
  ...
  Widget _buildDeviceDetail(BuildContext context) {
    List<UserHomeDevices> devices = widget.location.devices;
    ...
    //print("_buildDeviceDetail deviceId : ${this._deviceId}  deviceTypeCode :  $deviceTypeCode");

    return BlocBuilder<UserHomeBloc, UserHomeState>(builder: (context, UserHomeState state) {
      switch (deviceTypeCode) {
        case 'light':
